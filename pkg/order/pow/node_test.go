...
func TestNode_Start(t *testing.T) {
	...
	chainmetadata := &pb.ChainMeta{Height: 1, BlockHash: block.BlockHash, InterchainTxCount: 1}
	...
	order, err := NewNode(
		...
		order.WithGetChainMetaFunc(func() *pb.ChainMeta {
			return chainmetadata
		}),
		order.WithGetBlockByHeightFunc(func(height uint64) (*pb.Block, error) {
			return block, nil
		}),
	)
	require.Nil(t, err)
