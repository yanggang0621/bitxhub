...
func NewContext(tx pb.Transaction, txIndex uint64, data *pb.TransactionData, currentHeight uint64, ledger *ledger.Ledger, logger logrus.FieldLogger, meterWasm bool) *Context {
	return &Context{
		Caller:           tx.GetFrom(),
		Callee:           tx.GetTo(),
